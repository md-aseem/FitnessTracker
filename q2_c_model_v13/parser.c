...
//                           fprintf(LOGFILE,"\n BATTERY MODEL:                      %s",simmain->batteryModelName);
                        ...
                        case POWER_FILE_NAME:
                           ...
                           break;
                        case HVAC_PRESENT:
                           getString(f,simmain->hvacPresent);                      match = YES;
                           fprintf(LOGFILE,"\n IS AN HVAC PRESENT:                 %s",simmain->hvacPresent);
                           break;
                        case CYCLE_CALLOUT:
                           fprintf(LOGFILE,"\n\n --- FOUND CYCLE DEFINITION ---\n"); match = YES;
                           /* initialize cycle */
                           cyc = cycleListHead;
                           if(cyc == NULL){  cycleListHead = initializeCycle();  cyc = cycleListHead;  }
                           else{
                              while(NEXT(cyc) != NULL)  cyc = NEXT(cyc);
                              NEXT(cyc) = initializeCycle();
                              cyc = NEXT(cyc);  }

                           cyc->startTime  = getValue(f);
                           cyc->targetSoc1 = getValue(f);
                           cyc->waitTime   = getValue(f);
                           cyc->targetSoc2 = getValue(f);

                           fprintf(LOGFILE,"  Cycle Begins at                  %6.2f hours\n",cyc->startTime);
                           fprintf(LOGFILE,"  It will first go to SOC          %6.2f\n",cyc->targetSoc1);
                           fprintf(LOGFILE,"  Then it will wait for            %6.2f hours\n",cyc->waitTime);
                           fprintf(LOGFILE,"  And end by going to SOC          %6.2f\n",cyc->targetSoc2);
